                                              ...
                                              sensor_msgs::PointCloud2 out_cloud_R, sensor_msgs::PointCloud2 out_cloud_LR)
{
    ...
    for (int i = 0; i < pc_1_size; i++)
    {
        ...
        else if (pc_1.points[i].z < calc_height_ - 0.20) // 路面範囲より低い場合
        {
...
inline void IdentifyGutter::estimatePlane(const sensor_msgs::PointCloud2 in_sensor_cloud)
{
    ...
    *in_sensor_cloud_pcl_ptr = in_sensor_cloud_pcl; // 平面近似でポインタを用いるためPtrに変換
    ...
    gridarray.header.stamp = time_planer_end_;
    grid.header.stamp = time_planer_end_;
    grid.header.frame_id = "velodyne";
    grid.coefficients[0] = plane_[0];
    grid.coefficients[1] = plane_[1];
    grid.coefficients[2] = plane_[2];
    grid.coefficients[3] = plane_[3];
    grid.resolution = 20;
    grid.cells.resize(1);
    grid.cells[0].x=0.0;
    grid.cells[0].y=0.0;
    grid.cells[0].z=0.0;
    gridarray.grids.resize(1);
...
IdentifyGutter::IdentifyGutter() : nh_(), pnh_("~"), tf_listener_(tf_buffer_)
{
    ...
    this->distance_limit_msac_ = 15.0;   // MSACに用いる点群の半径範囲
...
void IdentifyGutter::Run()
{
    ...
    grid_pub = nh_.advertise<jsk_recognition_msgs::SimpleOccupancyGridArray>("gridarray", 10);
